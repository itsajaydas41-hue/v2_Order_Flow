...
var SB_TABLES = ["VendorMaster", "SKUMaster", "TransporterMaster", "SupplierMaster", "RawMaterialMaster", "PackingMaterialMaster", "PR", "PRItems", "Quotation", "QuotationItems", "Users", "Orders", "OrderItems", "Planning", "PlanItems", "GateEntry", "LoadItems", "Invoice", "GateOut", "POD", "PODItems", "Returns", "Collection", "Schedule", "ScheduleItems", "PO", "POItems", "SEN", "SENItems", "QC", "Receiving", "PurchaseReturn", "Payment", "Holidays", "FMS_O2C_Order", "FMS_O2C_Dispatch", "FMS_P2P_PO", "FMS_P2P_Inbound"];
...
const SHEETS = {
  ...
  SupplierMaster:    ['SupplierCode','SupplierName','SupplierType','BrokerName','Email','Address','ContactPerson','Mobile','GST','CreditDays'],
  RawMaterialMaster: ['RMCode','RMName','Brand','Category','UOM','GSTPercent','Rate'],
  /* Packing material — stock abhi manual/bulk se, baad me IMS API se aayega (structure wahi rahega) */
  PackingMaterialMaster: ['PMCode','PMName','Category','UOM','GSTPercent','Rate','MOQ','ReorderLevel','CurrentStock','StockUpdatedOn'],
  PR:                ['PRNo','PRDate','RequestedBy','Status','Remarks','CreatedAt'],
  PRItems:           ['PRNo','PMCode','PMName','UOM','CurrentStock','MOQ','ReorderLevel','RequiredQty','Remarks'],
  Quotation:         ['QuoteNo','PRNo','SupplierCode','SupplierName','QuoteDate','ValidUntil','DeliveryDays','TotalValue','Status','Remarks','CreatedAt'],
  QuotationItems:    ['QuoteNo','PRNo','PMCode','PMName','UOM','Qty','Rate','GSTPercent','Amount'],
  ...
  PO:                ['PONo','PODate','SupplierCode','SupplierName','BrokerName','SupplierEmail','MaterialType','PRNo','QuoteNo','ExpectedDeliveryDate','DeliveryConfirmedOn','TransportType','NumVehicles','DeductionCondition','PackingTerms','Remarks','TotalQty','TotalValue','Status','CreatedBy','CreatedAt'],
...
const PR_STATUS  = { OPEN:'Open', QUOTED:'Quotations Received', ORDERED:'PO Created', CLOSED:'Closed' };
const QUOTE_STATUS = { RECEIVED:'Received', SELECTED:'Selected', REJECTED:'Not Selected' };
const MATERIAL_TYPE = { RM:'Raw Material', PM:'Packing Material' };
const SUPPLIER_TYPE = { RM:'Raw Material', PM:'Packing Material', BOTH:'Both' };
/* Material type ke hisaab se supplier filter — 'Both' dono me aata hai.
   Purane suppliers jinme type nahi bhara, wo dono me dikhenge (data safe rahe). */
function suppliersFor_(materialType){
  const want = materialType===MATERIAL_TYPE.PM ? SUPPLIER_TYPE.PM : SUPPLIER_TYPE.RM;
  return readAll_('SupplierMaster').filter(function(s){
    const t=String(s.SupplierType||'').trim();
    if(!t) return true;                                  // type khaali = dono me
    return t===want || t===SUPPLIER_TYPE.BOTH;
  });
}
function getSuppliersByType(materialType){ return suppliersFor_(materialType); }
...
function uomOf_(skuCode){
  ...
  if(r) return r.UOM||'';
  const pm=readAll_('PackingMaterialMaster').find(function(x){ return String(x.PMCode)===String(skuCode); }); // packing material
  return pm ? (pm.UOM||'') : '';
}
function fyTag_(){
...
function getMasters(){
  return {
    ...
    packingmaterials: readAll_('PackingMaterialMaster')
  };
}
...
function savePackingMaterial(m){ append_('PackingMaterialMaster', m); return getMasters().packingmaterials; }
function deletePackingMaterial(code){ deleteWhere_('PackingMaterialMaster','PMCode',code); return getMasters().packingmaterials; }
...
const BULK_CFG = {
  ...
  PackingMaterialMaster: { key:'PMCode', required:['PMCode','PMName'] }
};
...
/* Packing material ke 3 reports */
function rptStock(){
  return getStockReport().map(function(r){
    return { 'PM Code':r.PMCode, 'PM Name':r.PMName, 'Category':r.Category, 'UOM':r.UOM,
             'Current Stock':r.CurrentStock, 'Reorder Level':r.ReorderLevel, 'MOQ':r.MOQ,
             'Rate':r.Rate, 'Stock Value':Math.round(r.CurrentStock*r.Rate),
             'Status':r.LowStock?'Low stock':'OK', 'Suggested Order Qty':r.SuggestedQty,
             'Stock Updated On':r.StockUpdatedOn };
  });
}
function rptPR(){
  const prs={}; readAll_('PR').forEach(p=>prs[p.PRNo]=p);
  const qc={}; readAll_('Quotation').forEach(q=>qc[q.PRNo]=(qc[q.PRNo]||0)+1);
  const po={}; readAll_('PO').forEach(p=>{ if(p.PRNo) po[p.PRNo]=p.PONo; });
  return readAll_('PRItems').map(function(i){
    const p=prs[i.PRNo]||{};
    return { 'PR Date':fmtDate_(p.PRDate), 'PR Number':i.PRNo, 'Requested By':p.RequestedBy||'',
             'PM Code':i.PMCode, 'PM Name':i.PMName, 'UOM':i.UOM||'',
             'Stock At PR':Number(i.CurrentStock)||0, 'Reorder Level':Number(i.ReorderLevel)||0, 'MOQ':Number(i.MOQ)||0,
             'Required Qty':Number(i.RequiredQty)||0, 'Quotations':qc[i.PRNo]||0,
             'PO Number':po[i.PRNo]||'', 'Status':p.Status||'' };
  });
}
function rptQuotation(){
  const qs={}; readAll_('Quotation').forEach(q=>qs[q.QuoteNo]=q);
  const po={}; readAll_('PO').forEach(p=>{ if(p.QuoteNo) po[p.QuoteNo]=p.PONo; });
  return readAll_('QuotationItems').map(function(i){
    const q=qs[i.QuoteNo]||{};
    return { 'Quote Date':fmtDate_(q.QuoteDate), 'Quote Number':i.QuoteNo, 'PR Number':i.PRNo,
             'Supplier Name':q.SupplierName||'', 'PM Code':i.PMCode, 'PM Name':i.PMName, 'UOM':i.UOM||'',
             'Qty':Number(i.Qty)||0, 'Rate':Number(i.Rate)||0, 'GST %':Number(i.GSTPercent)||0,
             'Amount':Number(i.Amount)||0, 'Delivery Days':Number(q.DeliveryDays)||0,
             'Quote Total':Number(q.TotalValue)||0, 'Status':q.Status||'', 'PO Number':po[i.QuoteNo]||'' };
  });
}
...
var REPORT_COLS = {
  ...
  StockReport:     ['PM Code','PM Name','Category','UOM','Current Stock','Reorder Level','MOQ','Rate','Stock Value','Status','Suggested Order Qty','Stock Updated On'],
  PRReport:        ['PR Date','PR Number','Requested By','PM Code','PM Name','UOM','Stock At PR','Reorder Level','MOQ','Required Qty','Quotations','PO Number','Status'],
  QuotationReport: ['Quote Date','Quote Number','PR Number','Supplier Name','PM Code','PM Name','UOM','Qty','Rate','GST %','Amount','Delivery Days','Quote Total','Status','PO Number']
};
/* Ye reports apne columns rakhte hain — 18-column normalise nahi hote */
var REPORT_CUSTOM_SHAPE = ['POReport','QCStatusReport','GRNReport','ReturnReport','POStatusReport','StockReport','PRReport','QuotationReport'];
...
function getReportRaw(type){
  ...
  switch(type){
    ...
    case 'StockReport':      return rptStock();
    case 'PRReport':         return rptPR();
    case 'QuotationReport':  return rptQuotation();
    default: return [];
  }
}
 ...
 * Stock abhi manual/bulk upload se aata hai. Aage IMS se jodna ho to
 * sirf updateStock() ko API se call karna hoga — baaki sab wahi rahega. */

/* ---- 1) Stock report ---- */
function getStockReport(){
  return readAll_('PackingMaterialMaster').map(function(m){
    const stock=Number(m.CurrentStock)||0, moq=Number(m.MOQ)||0, rl=Number(m.ReorderLevel)||0;
    const low = rl>0 ? stock<=rl : (moq>0 && stock<=moq);
    const suggested = low ? Math.max(moq, Math.max(rl*2-stock,0)) : 0;
    return { PMCode:m.PMCode, PMName:m.PMName, Category:m.Category||'', UOM:m.UOM||'',
             CurrentStock:stock, MOQ:moq, ReorderLevel:rl, Rate:Number(m.Rate)||0,
             GSTPercent:Number(m.GSTPercent)||0,
             StockUpdatedOn:m.StockUpdatedOn?fmtDate_(m.StockUpdatedOn):'',
             LowStock:low, SuggestedQty:Math.round(suggested) };
  });
}
/* Stock update — abhi manual, aage IMS API isi ko call karega */
function updateStock(p){
  const rows=(p&&p.rows)||[];
  let n=0;
  rows.forEach(function(r){
    if(!r.PMCode) return;
    updateWhere_('PackingMaterialMaster','PMCode',r.PMCode,{ CurrentStock:Number(r.CurrentStock)||0, StockUpdatedOn:new Date() });
    n++;
  });
  return { updated:n };
}
...
function savePR(p){
  const items=(p.items||[]).filter(it=>Number(it.requiredQty)>0);
  if(!items.length) throw new Error('Kam se kam ek item ki quantity daaliye.');
  const prNo=nextSeq_('PR','PRNo','PR/'+fyTag_()+'-',4);
  const stock={}; readAll_('PackingMaterialMaster').forEach(m=>stock[m.PMCode]=m);
  append_('PR',{ PRNo:prNo, PRDate:p.prDate||fmtDate_(new Date()), RequestedBy:currentUser_().name,
                 Status:PR_STATUS.OPEN, Remarks:p.remarks||'', CreatedAt:new Date() });
  appendMany_('PRItems', items.map(function(it){
    const m=stock[it.pmCode]||{};
    return { PRNo:prNo, PMCode:it.pmCode, PMName:it.pmName, UOM:m.UOM||'',
             CurrentStock:Number(m.CurrentStock)||0, MOQ:Number(m.MOQ)||0, ReorderLevel:Number(m.ReorderLevel)||0,
             RequiredQty:Number(it.requiredQty)||0, Remarks:it.remarks||'' };
  }));
  return { prNo:prNo };
}
function getPRs(){
  const qc={}; readAll_('Quotation').forEach(q=>qc[q.PRNo]=(qc[q.PRNo]||0)+1);
  return readAll_('PR').reverse().map(function(r){
    const its=readAll_('PRItems').filter(i=>i.PRNo===r.PRNo);
    return { PRNo:r.PRNo, PRDate:fmtDate_(r.PRDate), RequestedBy:r.RequestedBy||'', Status:r.Status,
             Items:its.length, TotalQty:its.reduce((s,i)=>s+(Number(i.RequiredQty)||0),0),
             Quotations:qc[r.PRNo]||0, Remarks:r.Remarks||'' };
  });
}
function getPRDetail(prNo){
  const pr=readAll_('PR').find(x=>x.PRNo===prNo); if(!pr) return null;
  return { pr:{ PRNo:pr.PRNo, PRDate:fmtDate_(pr.PRDate), RequestedBy:pr.RequestedBy||'', Status:pr.Status, Remarks:pr.Remarks||'' },
           items:readAll_('PRItems').filter(i=>i.PRNo===prNo) };
}
/* PR jispar abhi PO nahi bana — quotation ke liye */
function getOpenPRs(){
  return readAll_('PR').filter(r=>r.Status!==PR_STATUS.ORDERED && r.Status!==PR_STATUS.CLOSED)
    .reverse().map(r=>({ PRNo:r.PRNo, PRDate:fmtDate_(r.PRDate), Status:r.Status }));
}

/* ---- 3) Quotations ---- */
function saveQuotation(p){
  const pr=readAll_('PR').find(x=>x.PRNo===p.prNo);
  if(!pr) throw new Error('Pehle Purchase Requisition banaiye — PR nahi mila.');   // CHAIN
  if(pr.Status===PR_STATUS.ORDERED) throw new Error('Is PR par PO ban chuka hai.');
  const items=(p.items||[]).filter(it=>Number(it.qty)>0 && Number(it.rate)>0);
  if(!items.length) throw new Error('Kam se kam ek item ka qty aur rate daaliye.');
  if(p.supplierCode){
    const sup=readAll_('SupplierMaster').find(s=>String(s.SupplierCode)===String(p.supplierCode));
    const st=sup?String(sup.SupplierType||'').trim():'';
    if(st && st!==SUPPLIER_TYPE.BOTH && st!==SUPPLIER_TYPE.PM)
      throw new Error('Quotation ke liye Packing Material supplier chunein — ye supplier "'+st+'" ka hai.');
  }
  const qNo=nextSeq_('Quotation','QuoteNo','QT/'+fyTag_()+'-',4);
  let tv=0;
  const rows=items.map(function(it){
    const qty=Number(it.qty)||0, rate=Number(it.rate)||0, gst=Number(it.gstPercent)||0;
    const amt=qty*rate*(1+gst/100); tv+=amt;
    return { QuoteNo:qNo, PRNo:p.prNo, PMCode:it.pmCode, PMName:it.pmName, UOM:uomOf_(it.pmCode),
             Qty:qty, Rate:rate, GSTPercent:gst, Amount:Math.round(amt) };
  });
  append_('Quotation',{ QuoteNo:qNo, PRNo:p.prNo, SupplierCode:p.supplierCode||'', SupplierName:p.supplierName,
    QuoteDate:p.quoteDate||fmtDate_(new Date()), ValidUntil:p.validUntil||'', DeliveryDays:Number(p.deliveryDays)||0,
    TotalValue:Math.round(tv), Status:QUOTE_STATUS.RECEIVED, Remarks:p.remarks||'', CreatedAt:new Date() });
  appendMany_('QuotationItems', rows);
  if(pr.Status===PR_STATUS.OPEN) updateWhere_('PR','PRNo',p.prNo,{Status:PR_STATUS.QUOTED});
  return { quoteNo:qNo };
}
/* Ek PR ke saare quotations — side by side compare ke liye */
function getQuotationsFor(prNo){
  const quotes=readAll_('Quotation').filter(q=>q.PRNo===prNo);
  const allItems=readAll_('QuotationItems').filter(i=>i.PRNo===prNo);
  const prItems=readAll_('PRItems').filter(i=>i.PRNo===prNo);
  return {
    prNo:prNo,
    prItems:prItems.map(i=>({ PMCode:i.PMCode, PMName:i.PMName, UOM:i.UOM||'', RequiredQty:Number(i.RequiredQty)||0 })),
    quotes: quotes.map(function(q){
      return { QuoteNo:q.QuoteNo, SupplierCode:q.SupplierCode||'', SupplierName:q.SupplierName,
               QuoteDate:fmtDate_(q.QuoteDate), ValidUntil:q.ValidUntil?fmtDate_(q.ValidUntil):'',
               DeliveryDays:Number(q.DeliveryDays)||0, TotalValue:Number(q.TotalValue)||0,
               Status:q.Status, Remarks:q.Remarks||'',
               items:allItems.filter(i=>i.QuoteNo===q.QuoteNo).map(i=>({ PMCode:i.PMCode, Qty:Number(i.Qty)||0, Rate:Number(i.Rate)||0, GSTPercent:Number(i.GSTPercent)||0, Amount:Number(i.Amount)||0 })) };
    })
  };
}
/* Aap khud select karenge — koi auto suggestion nahi */
function selectQuotation(quoteNo){
  const q=readAll_('Quotation').find(x=>x.QuoteNo===quoteNo); if(!q) throw new Error('Quotation not found');
  readAll_('Quotation').filter(x=>x.PRNo===q.PRNo).forEach(function(x){
    updateWhere_('Quotation','QuoteNo',x.QuoteNo,{ Status: x.QuoteNo===quoteNo ? QUOTE_STATUS.SELECTED : QUOTE_STATUS.REJECTED });
  });
  return { ok:true, quoteNo:quoteNo, prNo:q.PRNo };
}
/* Selected quotation se PO banane ke liye ready data */
function getSelectedQuotes(){
  const sel=readAll_('Quotation').filter(q=>q.Status===QUOTE_STATUS.SELECTED);
  const prs={}; readAll_('PR').forEach(r=>prs[r.PRNo]=r);
  return sel.filter(q=>{ const pr=prs[q.PRNo]; return pr && pr.Status!==PR_STATUS.ORDERED; })
    .map(function(q){
      return { QuoteNo:q.QuoteNo, PRNo:q.PRNo, SupplierCode:q.SupplierCode||'', SupplierName:q.SupplierName,
               DeliveryDays:Number(q.DeliveryDays)||0, TotalValue:Number(q.TotalValue)||0,
               items:readAll_('QuotationItems').filter(i=>i.QuoteNo===q.QuoteNo)
                 .map(i=>({ skuCode:i.PMCode, skuName:i.PMName, uom:i.UOM||'', qty:Number(i.Qty)||0, rate:Number(i.Rate)||0, gstPercent:Number(i.GSTPercent)||0 })) };
    });
}
/* ---- 5) Delivery date confirm ---- */
function confirmPODelivery(p){
  const po=readAll_('PO').find(x=>x.PONo===p.poNo); if(!po) throw new Error('PO not found');
  if(po.Status===PO_STATUS.DRAFT) throw new Error('Pehle PO supplier ko bhejein, uske baad delivery date confirm karein.');
  if(!p.expectedDate) throw new Error('Expected delivery date daaliye.');
  updateWhere_('PO','PONo',p.poNo,{ ExpectedDeliveryDate:p.expectedDate, DeliveryConfirmedOn:new Date() });
  return { ok:true };
}

/* Prefix ke hisaab se alag ginti — PO/SRFM26-0001 aur POP/SRFM26-0001 dono independent */
function nextSeqPrefixed_(table, col, prefix, pad){
  const rows=readAll_(table).filter(function(r){ return String(r[col]||'').indexOf(prefix)===0; });
  let mx=0;
  rows.forEach(function(r){ const n=parseInt(String(r[col]).slice(prefix.length),10); if(!isNaN(n)&&n>mx) mx=n; });
  return prefix+String(mx+1).padStart(pad||4,'0');
}

/* ---- Purchase Order ---- */
function savePO(p){
  const mtype = p.materialType===MATERIAL_TYPE.PM ? MATERIAL_TYPE.PM : MATERIAL_TYPE.RM;
  /* CHAIN (sirf packing material par): PR → quotation select → tabhi PO */
  if(mtype===MATERIAL_TYPE.PM){
    if(!p.quoteNo) throw new Error('Packing material ka PO selected quotation se hi banta hai — pehle quotation select karein.');
    const q=readAll_('Quotation').find(x=>x.QuoteNo===p.quoteNo);
    if(!q) throw new Error('Quotation nahi mila.');
    if(q.Status!==QUOTE_STATUS.SELECTED) throw new Error('Ye quotation abhi select nahi hui hai.');
    if(readAll_('PO').some(x=>x.QuoteNo===p.quoteNo)) throw new Error('Is quotation par PO pehle hi ban chuka hai.');
    p.prNo=q.PRNo;
  }
  /* Supplier us material type ka hona chahiye */
  if(p.supplierCode){
    const sup=readAll_('SupplierMaster').find(s=>String(s.SupplierCode)===String(p.supplierCode));
    const st=sup?String(sup.SupplierType||'').trim():'';
    if(st && st!==SUPPLIER_TYPE.BOTH && st!==mtype)
      throw new Error('Ye supplier "'+st+'" ka hai — '+mtype+' ke liye doosra supplier chunein.');
  }
  /* Alag series: packing = POP/…, raw material = PO/… */
  const prefix = (mtype===MATERIAL_TYPE.PM ? 'POP/' : 'PO/') + fyTag_() + '-';
  const poNo = mtype===MATERIAL_TYPE.PM
    ? nextSeqPrefixed_('PO','PONo',prefix,4)
    : nextSeqPrefixed_('PO','PONo',prefix,4);
  let tq=0,tv=0;
  ...
  append_('PO',{PONo:poNo,PODate:p.poDate||fmtDate_(new Date()),SupplierCode:p.supplierCode||'',SupplierName:p.supplierName,BrokerName:p.brokerName||'',SupplierEmail:p.supplierEmail||'',MaterialType:mtype,PRNo:p.prNo||'',QuoteNo:p.quoteNo||'',ExpectedDeliveryDate:p.expectedDeliveryDate||'',TransportType:p.transportType||'',NumVehicles:Number(p.numVehicles)||0,DeductionCondition:p.deductionCondition||'',PackingTerms:p.packingTerms||'',Remarks:p.remarks||'',TotalQty:tq,TotalValue:Math.round(tv),Status:PO_STATUS.DRAFT,CreatedBy:u.email,CreatedAt:new Date()});
  appendMany_('POItems',items);
  if(mtype===MATERIAL_TYPE.PM && p.prNo) updateWhere_('PR','PRNo',p.prNo,{Status:PR_STATUS.ORDERED});
  try{ const _n=new Date();
...
function getPOs(){ return readAll_('PO').reverse().map(p=>({PONo:p.PONo,PODate:fmtDate_(p.PODate),SupplierName:p.SupplierName,BrokerName:p.BrokerName||'',SupplierEmail:p.SupplierEmail||'',MaterialType:p.MaterialType||MATERIAL_TYPE.RM,PRNo:p.PRNo||'',QuoteNo:p.QuoteNo||'',ExpectedDeliveryDate:p.ExpectedDeliveryDate?fmtDate_(p.ExpectedDeliveryDate):'',TotalQty:p.TotalQty,TotalValue:p.TotalValue,Status:p.Status})); }
...
function getPODetail(poNo){
  ...
  return {po:{PONo:po.PONo,PODate:fmtDate_(po.PODate),SupplierName:po.SupplierName,SupplierCode:po.SupplierCode,
              ...
              MaterialType:po.MaterialType||MATERIAL_TYPE.RM,PRNo:po.PRNo||'',QuoteNo:po.QuoteNo||'',ExpectedDeliveryDate:po.ExpectedDeliveryDate?fmtDate_(po.ExpectedDeliveryDate):'',
